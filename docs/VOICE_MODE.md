...
The Audio control beside Pause/Resume and Cancel displays the current output and opens a sheet showing the actual microphone and output reported by `AVAudioSession.currentRoute`. iOS 26 uses `AVInputPickerInteraction` for microphone selection; iOS 18 uses the session's available-input list. Output selection uses `AVRoutePickerView`, with explicit phone-speaker and automatic-routing actions. Bluetooth hands-free input/output are coupled, and the speaker action also selects the built-in microphone. The app does not promise arbitrary microphone/speaker combinations or voice compatibility with every AirPlay/car accessory.

`AudioRouteController` owns routing intent, confirmation timeouts, interruption state, and the published route through an injectable `VoiceAudioSession` adapter. `IOSVoiceAudioSession` owns notification tokens and native APIs. Activation and deactivation run on a serial background queue to keep blocking hardware calls off the main actor on iOS 18 and later. The controller orders teardown before the next activation and rejects completions from stopped sessions; an interruption during Resume still requires another explicit Resume. These settings remain local to the audio session. `VoiceAgentAudioRouting` coordinates changes with both engines: it stops capture and queued playback before switching, interrupts the current spoken response without cancelling submitted coding work, and rebuilds the shared VoiceProcessingIO unit when effective I/O changes. Capture callbacks from a retired unit are discarded. Opening a picker while idle does not wake a paid voice connection.

Losing an active headphone/Bluetooth route places voice in an explicit suspension until Resume. Incoming audio, completion announcements, event-driven wake, and ready chimes cannot clear this hold; reconnecting headphones does not resume automatically. System interruptions and media-services loss/reset also require Resume. Removal is detected asynchronously, so this is not a guarantee that zero frames can reach a fallback output before the notification arrives. Physical-device accessory routing and echo cancellation require listening tests; simulator policy tests do not establish those behaviors.
