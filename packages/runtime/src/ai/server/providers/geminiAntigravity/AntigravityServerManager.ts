 ...
 *
 * Owns the lifecycle of the Antigravity language server that backs the
 * `antigravity-gemini-agent` provider, and the Connect-RPC calls made against
 * it. Configuration (the supported-build floor and the spawn port candidates)
 * is injected via configure() rather than baked in, so the floor can be raised
 * from settings when Antigravity ships a new required version.
 *
...
// Defaults used when nothing calls configure() (tests, dev harness). The
// version floor is what the language server enforces against
// --override_ide_version; raise it here when Antigravity rejects the build.
...
export class AntigravityServerManager {
  ...
  // Cross-process lock so concurrent Nimbalyst processes (a second dev
  // instance, a packaged build alongside dev, a restart overlapping the old
  // process) share ONE language server instead of each spawning their own and
  ...
  /**
   * Apply externally-supplied configuration. Idempotent. Safe to call before or
  ...
  private rpc<T = any>(method: string, body: unknown, ep: AntigravityEndpoint,
    timeoutMs = 120_000, abortSignal?: AbortSignal): Promise<T> {
    ...
    return new Promise<T>((resolve, reject) => {
      ...
      // Cancellation. GetModelResponse is buffered, so an abandoned turn would
      // otherwise keep generating for the full timeout and only then resolve
      // into a void. Destroying the request settles it immediately, and the
      // server survives it -- a Heartbeat straight after a mid-flight destroy
      // answers normally, so no respawn is needed.
      if (abortSignal) {
  ...
  async getModelResponse(prompt: string, modelKeyOrEnum: string,
    timeoutMs = 120_000, abortSignal?: AbortSignal): Promise<string> {
    ...
    for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
      ...
      try {
        ...
        // A cancelled turn is not a transport fault -- retrying it would run
        // the generation the user just stopped.
