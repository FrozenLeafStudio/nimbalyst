 ...
 * provider.
 *
 * This is NOT an `AgentProtocol` implementation, and deliberately so. The
 * language server exposes one stateless RPC -- `GetModelResponse({prompt,
 * model}) -> {response}`. There is no conversation id, no streaming, no
 * function-calling surface, and no token accounting. Every turn re-renders the
 * ENTIRE conversation into a single flat prompt, which is why the history
 * budgeting below is load-bearing rather than an optimization.
 *
...
export class AntigravityToolLoopProtocol {
  ...
  // Signal for THIS turn, handed to every GetModelResponse so an abort tears
  // down the in-flight HTTPS request instead of letting a buffered generation
  // run to its timeout after the user has already moved on. `this.aborted`
  // stops the loop; this stops the request the loop is blocked on.
  ...
  private async finalizeFromContext(fullSystemPrompt: string, timeoutMs: number): Promise<string> {
    try {
      ...
      const finalPrompt = this.renderPrompt(fullSystemPrompt);
      const resp = await this.server.getModelResponse(finalPrompt, this.modelKey, timeoutMs, this.currentAbortSignal);
      if (this.aborted) return '';
  ...
  private async verifyFinalAnswer(draft: string, timeoutMs: number): Promise<string> {
    ...
    try {
      const resp = await this.server.getModelResponse(verifyPrompt, this.modelKey, timeoutMs, this.currentAbortSignal);
